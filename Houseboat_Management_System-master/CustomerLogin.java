...
public class CustomerLogin implements ActionListener{
    ...
    CustomerLogin(){
		...
		f.getContentPane().setBackground(new Color(142, 167, 233));
		f.setLayout(new BorderLayout(30,30));
		
		Font ff = new Font(Font.MONOSPACED,Font.BOLD,18);
		//menubar
		m = new JMenuBar();
		m.setBackground(new Color(114, 134, 211));//Giving color--Comes under swing 
	    ...
	    JLabel title = new JLabel("CUSTOMER LOGIN PAGE",JLabel.CENTER);
        ...
        btnNewButton.addActionListener(new ActionListener() {
        	 public void actionPerformed(ActionEvent e) {
                 ...
                 try {
                     Connection connection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/House Boat","postgres","8500287327");

                     PreparedStatement st = (PreparedStatement) connection
                         .prepareStatement("Select username,password from Login_Customer where username =? and password =?");
